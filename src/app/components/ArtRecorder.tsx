...
export default function ArtRecorder() {
  const [isRecording, setIsRecording] = useState(false);
  const [audioBlob, setAudioBlob] = useState<Blob | null>(null);
  const [photo, setPhoto] = useState<File | null>(null);
  const [walletNumber, setWalletNumber] = useState(''); // State to hold the wallet number
  ...
  const sendData = async () => {
    ...
    formData.append('walletNumber', walletNumber); // Add wallet number to the FormData
    if (audioBlob) {
    ...
    try {
      const response = await fetch('https://hook.us2.make.com/eobms0je6oxvikcjq48bq1w2j8cfrzmf', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        setAudioBlob(null);
        setPhoto(null);
        setWalletNumber(''); // Reset the wallet number
      ...
      alert(`Failed to send data. Please try again. Error: ${errorMessage}`);
    } finally {
      setIsSending(false);
    }
  };
  ...
  return (
    <div>
        ...
        <button onClick={reloadPage} className="text-blue-800">
          ...
          >
            <path
              ...
              strokeWidth={2}
              d="M3 12l2-2m0 0l7-7 7 7m-9-5v12m12 0h-3m-6 0h-3m9-7v7"
            />
          </svg>
        </button>
      </div>
      ...
      {/* Title */}
      <h1 className="text-2xl font-semibold text-center mb-5 text-blue-800">Certificado de Arte</h1>
